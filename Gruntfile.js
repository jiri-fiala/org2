...
var PathConfig = require('./grunt-settings.js');
  ...
  grunt.initConfig({
    ...
    clean: {
      options: { force: true },
      temp: {
        src: ["<%= config.cssDir %>**/*.map", "<%= config.imgDir %>", "<%= config.distDir %>/*", "<%= config.cssMainFileDir %><%= config.cssMainFileName %>.css.map", "./jpgtmp.jpg"]
      }
    },
...
//finally
  ...
  //final build
  grunt.registerTask('dist', ['clean:temp', 'imgmin', 'cssbeauty', 'copy:dist']);
